...
artifacts = joblib.load("./app/models/bonprix_recommender_model.joblib")
transformer = artifacts["content_transformer"]
interactions = artifacts["interactions"]
content_matrix = artifacts["content_matrix"]          
product_id_to_idx = artifacts["product_id_to_idx"]    # Changed key from 'crate_barrel_product_id_to_idx' to 'product_id_to_idx'

# 2. Load your freshly scraped Bonprix JSON products
with open("./web-scraping/Scraped_data/bonprix_departments.json", "r", encoding="utf-8") as f:
    scraped_data = json.load(f)

# Convert JSON dict-of-dicts to DataFrame
json_df = pd.DataFrame(list(scraped_data.values()))

# Helper to clean price (" ab 17,90 € UVP 22,90" -> 17.90)
def clean_price(val):
    if pd.isna(val):
        return 0.0
...
aligned_df["Department"] = json_df["Department"]  # Mapped to Bonprix's 'Department'
...
def recommend_json_to_customer(customer_id: str, top_n: int = 3):
    # Get customer's positive Bonprix interactions
    ...
    # Build the Bonprix User Taste Profile
    ...
    weights = cust_history.loc[
        cust_history["Product_ID"].isin(product_id_to_idx), "score"
    ].to_numpy()
    weights = weights / weights.sum()

    user_profile = np.average(content_matrix[idxs], axis=0, weights=weights).reshape(
        1, -1
    )
    ...
    # Return using Bonprix's specific JSON columns
    return results[
...
sample_customer = interactions["Customer_ID"].iloc[47]
